...
import rdf, { isNamedNode, isNode, Literal } from '@ontologies/core';
...
const DashboardFull: FC<PropTypes> = ({ dashboard }) => {
  useDataFetching([dashboard]);
  const [itemSequence] = useResourceProperty(dashboard, ontola.menuItems);
  const items = useContainerToArr(isNamedNode(itemSequence) ? itemSequence : undefined);
  const [firstItem] = useResourceProperty(isNamedNode(itemSequence) ? itemSequence : undefined, rdfx.ns('_0'));
  const [currentTab, setCurrentTab] = useState<SomeNode | undefined>(undefined);
  const [iriTemplate] = useResourceProperty(isNamedNode(currentTab) ? currentTab : undefined, ontola.href);
