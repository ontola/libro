...
import useHandleVote from '../../hooks/votes/useHandleVote';
...
const PollOption = ({
  ...
  const [handleClick, loading] = useHandleVote(action, option, currentOption);

  const [name] = useStrings(option, schema.name);
  const voted = !rdf.equals(currentOption, argu.abstain);
  const progress = 100 * (count / totalCount);
