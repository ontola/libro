...
export const useHasInteraction = (collectionResource: SomeNode): boolean => {
  ...
  const actionStatusses = useGlobalIds(actions, schema.actionStatus).map(([_, status]) => status);
  ...
  return actionStatusses.findIndex((actionStatus) => !isInvisibleActionStatus(actionStatus)) !== -1;
};
