...
export default [
  router.get('/api/persons', (req, res) => {
    res.json({
      links: {
        self: 'http://localhost:3000/api/persons',
      },
      data: persons,
    });
  }),
  router.get('/api/persons/:personId', (req, res) => {
    res.json({
      links: {
        self: `http://localhost:3000/api/persons/${req.params.personId}`,
      },
      data: persons.find(o => o.id === req.params.personId),
    });
  }),
];
