...
* **locales** - Stores internationalization data and current translations. See the I18n chapter.
  * **app** - Autogenerated extractions from the `app` folder.
* **server** - Node server
...
extraction and file management is done with [mady](https://github.com/guigrpa/mady).

To create and verify translations:
* Run `mady` (or `./node_modules/.bin/mady`)
* The browser will open showing current translations
* Close the 'en' column, these are populated automatically.
* Fill in any of the missing translations.

***CAUTION:*** When deleting translations (e.g. removing a react-intl component), make sure to 
manually check and/or update the translations files to ensure consistency.
